...
Public release. Base mod, Proper4KUI Patch, and Ruler Designer.
...
### Removed
- Dead code `revoke_allowed` blocks from decisions.
...
### Changed
- Add Steam Workshop and Paradox Forums links.
...
### Removed
- Untrack `descriptor.mod` across the four SCME mod folders.
...
### Added
- **README Community section** — links to GitHub Issues and Discussions.
- **`Traits` tag** on `SketchyCheatMenuExtended.mod` for Steam Workshop discoverability.
- **README "What is new" section** — summary of what this fork adds compared to Sketchy Cheat Menu.

### Changed
- CHANGELOG's `[Unreleased]` header restored (had been dropped since v0.8.2).

## [0.14.4] - 2026-07-07

### Changed
- **Rebrand.** Renamed from Sketchy Cheat Menu Plus to Sketchy Cheat Menu Extended to avoid confusion with the CK2+ (CK2Plus) submod naming convention. Folders, `.mod` descriptors, script identifiers (`scmp_` → `scme_`), and debug event namespace (`SCMPD.*` → `SCMED.*`) all updated.
- Thumbnail, banner, and social preview regenerated with the new name.

### Fixed
- Encoding corruption in comments across four files.
...
### Changed
- **12 `add_all_*` decision effect bodies** extracted into matching `add_all_*_effect` scripted_effects.
...
### Changed
- README rewrite — in-game labels throughout, restructured "What it does" sections, Optional DLC uses `Hidden` / `Stays` sub-bullets.

## [0.12.4] - 2026-07-01

### Added
- README banner, screenshots gallery, and GitHub social preview under `docs/`.
- AGENTS.md — contributor orientation guide.

## [0.12.3] - 2026-06-30

### Changed
- Updated the mod thumbnail for SCME.
...
### Added
- **Proper4KUI Patch trait icons** (`SketchyCheatMenuExtendedProper4KUIPatch/gfx/traits/`) — 43×43 hi-res variants of the 5 SCME-custom cheat trait icons.

### Changed
- Redesigned 5 SCME-custom cheat trait icons (Beloved, Master Builder, Master Commander, Master Landowner, Master Plotter), preserving Sketchy 2019's gold-ring + dark-purple-disk framework.
...
### Fixed
- 5 SCME-custom trait DDS files were 24×25 (Sketchy 2019 off-by-one); resaved at 24×24.
- `enable_triple_levies` description double-period typo.

## [0.12.0] - 2026-06-29

### Added
- **Proper4KUI Patch sub-mod** (`SketchyCheatMenuExtendedProper4KUIPatch`) — companion mod that swaps in the 50×50 hi-res icon variant when [Proper 4K UI Project](https://steamcommunity.com/sharedfiles/filedetails/?id=3054987840) is also loaded.
...
- 5 new scripted_triggers in `scme_law_triggers.txt` (3 tier-eligibility helpers + 2 decision visibility triggers).
...
- 1 new section-bulk scripted_effect in `scme_bulk_effects.txt` (`clear_cheat_traits_effect`) and 2 new visibility scripted_triggers in `scme_trait_section_triggers.txt`.
...
- 3 new section-bulk scripted_effects in `scme_bulk_effects.txt` (`clear_leadership_traits_effect`, `clear_warriors_traits_effect`, `clear_chinese_imperial_traits_effect`) and 6 new visibility scripted_triggers in `scme_trait_section_triggers.txt`.
...
- 2 new scripted_effects in `scme_bulk_effects.txt` (`clear_sympathy_traits_effect`, `clear_sympathy_mutex_effect`) and 2 new visibility scripted_triggers in `scme_trait_section_triggers.txt` (`has_any_sympathy_trait_trigger`, `has_all_sympathy_traits_trigger` — religion-aware via trigger_if branches per religion_group).
- 2 new GFX sprite entries in `cheats_menu_intrigue_traits.gfx`.
...
- 2 new section-bulk scripted_effects in `scme_bulk_effects.txt` (`clear_lifestyle_traits_effect`, `clear_lifestyle_mutex_effect`) and 2 new visibility scripted_triggers in `scme_trait_section_triggers.txt`.
...
- 5 new section-bulk scripted_effects in `scme_bulk_effects.txt` (`clear_positive_personality_effect`, `clear_negative_personality_effect`, `clear_childhood_traits_effect`, `clear_positive_personality_mutex_effect`, `clear_negative_personality_mutex_effect`) and 6 new visibility scripted_triggers in `scme_trait_section_triggers.txt`.
...
- 3 new section-bulk scripted_effects in `scme_bulk_effects.txt` (`clear_virtue_traits_effect`, `clear_sin_traits_effect`, `clear_virtue_mutex_effect`) and 4 new visibility scripted_triggers in `scme_trait_section_triggers.txt` (`has_any_virtue_trigger`, `has_all_virtues_trigger`, `has_any_sin_trigger`, `has_all_sins_trigger`).
...
### Added
- **Build extras** — 3 new decisions (`scme_build_hospital` / `build_fort` / `build_trade_post`) build a hospital / fort / trade post in the county. Right-click the county capital; `scme_build_hospital` requires Reaper's Due, `build_fort` requires Conclave, `build_trade_post` requires The Republic. `scme_build_hospital` carries the `scme_` prefix to avoid colliding with vanilla's `title_decisions.build_hospital`.
...
- **SCMED.16 debug event** — sets a province's culture to nahua (CleanSlate) or nahuatl (vanilla); used to test the stack-aware Aztec tribal helper.
...
- **14 SCMED debug events** (`SCMED.2`-`SCMED.15`) in the Debug sub-mod for v0.9.1 testing: HF-bloodline grants, Hellenic temple dedications, `tomb_of_a_saint` modifier, and province-culture overrides.

### Changed
- README updated for the new conditional adds and SCMED debug events.
...
### Fixed
- **CleanSlate culture compatibility** — `add_castle_culture_buildings_effect` and its trigger now handle both vanilla and CleanSlate culture names via new compat triggers in `scme_culture_compat_triggers.txt`. 6 culture branches were silently failing under CleanSlate due to vanilla-only culture names. The byzantine branch now also correctly excludes Coptic and Arberian, which have dedicated buildings.

## [0.9.0] - 2026-06-20

### Changed
- **Helper file reorganization** — `scme_holding_effects.txt` and `scme_holding_triggers.txt` split by holding type. New files: `scme_castle_effects.txt`, `scme_temple_effects.txt`, `scme_city_effects.txt`, `scme_tribal_effects.txt`, `scme_nomad_effects.txt`, `scme_castle_triggers.txt`. Helper bodies byte-identical; no behavioral change.
...
- **Style guide conformance audit** — mechanical normalizations across 21 files:
  ...
  - §2.6 Section header style — 4 outlier headers normalized to SCME's `## --- TEXT --- ##` style.
...
### Fixed
- Em-dash → en-dash sweep across 5 files (`scme_government_compat_triggers.txt`, `cheats_menu_intrigue_spawn.txt`, `disown_them.txt`, `cheats_religion_modifiers.txt`, `scme_debug_events.txt`).
...
- **CleanSlate compat helper** — `has_scme_inheritance_law_eligible_government_trigger` gates the `show_inheritance_laws` master toggle to the 8 govs SCME has tested (feudal / tribal / nomadic / iqta / monastic_feudal / chinese_imperial / order / merchant_republic).
...
### Added
- **Mod-detection global flag** `scme_active` set at `on_startup`, parallel to Elite Recruitment's `elite_recruitment_active` and the `cleanslate_active` flag this mod reads for trait-name compatibility. New file `common/on_actions/scme_on_startup.txt`.
- **Persistent `scme_spawned_<role>` flag** on every spawn-template character. 11 flag names across 25 templates — `scme_spawned_child` (6 child templates), `scme_spawned_sibling` (10 sibling templates), and one each for wife / vassal / marshal / spymaster / steward / chancellor / priest / commander / physician. Survives saves; not cleared by the existing finalize hook. Enables future bulk-cleanup decisions, bloodline-grant scoping, and inter-mod identification.
...
- **`cheat_trait_cleaner` description loc clarified** — lists the 6 SCME cheat traits it strips and notes dynasty preservation.
- **README** — new "Mod detection" subsection under "For modders" documenting `scme_active` and the per-character `scme_spawned_<role>` flags.
...
- **New cluster helper** `clear_kowtow_traits_effect` (3-way mutex) — vanilla declares no opposites between the kowtow tiers, but SCME tightens to strict mutex matching the raiding / bloodthirsty_gods convention.
...
- **Three new cluster helpers** in `scme_mutex_effects.txt` for the mutex groupings.
...
- **Two new cluster helpers** in `scme_mutex_effects.txt`: `clear_coronation_traits_effect` + `clear_baptism_traits_effect` (4-way each).
...
- **`has_ordained_clergy_religion_trigger`** in new `scme_section_triggers.txt` — gates section show toggle to the 4 religions that have ordained-clergy traits. Hide stays flag-keyed so chars who converted away can still clean up.
...
### Changed
- **Carved out** `common/scripted_triggers/scme_section_triggers.txt` for religion-set section-gate triggers. Moved 3 triggers out of `scme_trait_compat_triggers.txt` (which now holds purely CleanSlate trait/religion rename compat): `has_warrior_lodge_religion_trigger`, `has_holy_war_religion_trigger`, `has_ordained_clergy_religion_trigger`. File-level reorganization; no runtime change — scripted_triggers are global once loaded.
...
- **Two new cluster helpers** in `scme_mutex_effects.txt`: `clear_bloodthirsty_gods_traits_effect` (3-way) and `clear_pagan_branch_traits_effect` (4-way).
...
- **New file** `common/scripted_effects/scme_mutex_effects.txt` — 5 cluster helpers for the multi-way mutex groupings (intellect / scarred / kinslayer / raiding / holy war rewards). Each cluster's `add_*` decisions invoke its helper before adding the trait.
- **Two strict compat helpers** (in `scme_trait_compat_triggers.txt` + `scme_trait_compat.txt`) for sex-agnostic seducer/seductress mutex — needed by `add_celibate` (strips both lifestyle sex-variants) and the cross-sex wraps on `add_seducer` / `add_seductress`. The existing sex-aware compat returns the sex-matched variant; the strict versions check / remove specifically.
...
- **Tooltip cleanup pass** — wrapped 224 unconditional `remove_trait = X` calls in `if = { limit = { trait = X } remove_trait = X }` so tooltips only render the removes that actually fire on the target character. Mechanical refactor; identical behavior, cleaner UX. Affected sites:
  - All 5 bulk-clear helpers in `scme_health_effects.txt` (61 wraps)
  - `clear_zodiac_traits_effect` in `scme_zodiac_effects.txt` (12 wraps — fixes v0.6.7 tooltip noise on `add_zodiac_*`)
  - All 16 compat helpers in `scme_trait_compat.txt` (48 wraps — wrapping removes inside the CleanSlate-routed if/else branches)
...
- **Consolidated two long-standing inline-duplication patterns** (flagged in code comments since v0.3.1, never deduped until now):
  - **`clear_education_traits_effect`** — new helper in new file `scme_education_effects.txt`. The 5 `set_X_education` cascades now call the helper instead of each carrying their own ~20 inline `if/limit` blocks. Cascade effects shrink from ~85 lines to 3. Order also normalized to remove-then-add (matching the consort-pair / zodiac convention).
  - **`clear_birth_defects_effect`** — new helper added to `scme_health_effects.txt`. `remove_defects`, `self_genes`, and `fix_genes` now all call the helper instead of carrying inline copies of the same 11-trait list. `self_genes` and `fix_genes` also gain filtered tooltips (their inline removes weren't wrapped in v0.6.8's first pass).
...
- **Zodiac section** between Birth Markers and Kinslayer — 12 trait pairs covering the Western zodiac signs in astrological calendar order (Aries → Pisces). Ungated per established SCME convention (v0.6.1 berserker, etc.): single-religion vanilla content with no cross-religion alternative stays ungated in the cheat menu. Strict one-zodiac-at-a-time mutex via new `clear_zodiac_traits_effect` helper invoked by each `add_zodiac_*` effect — vanilla's pair-only `opposites` (Aries↔Libra etc.) isn't strict enough when the cheat menu bypasses natural one-sign-at-birth assignment.
- **New scripted_effects file** `scme_zodiac_effects.txt` — contains the `clear_zodiac_traits_effect` helper (removes all 12 zodiac traits).
...
- **12 new SCME localisation entries** in `cheats_menu_intrigue_traits.csv` for the 6 cycle decisions (main + desc).
...
- **22 new SCME localisation entries** in `cheats_menu_intrigue_traits.csv` for the 11 new decision pairs.
...
- **24 new SCME localisation entries** in `cheats_menu_intrigue_traits.csv` for the 12 new decision keys (`add_*` / `remove_*` labels plus `_desc` tooltips).
...
### Added
- **17 new trait-toggle decision pairs.** Single-stack base-game traits get direct `add_trait`/`remove_trait`; CleanSlate-renamed traits route through new compat helpers in `scme_trait_compat.txt` + `scme_trait_compat_triggers.txt`. See diff for specifics.
...
### Added
- **`SketchyCheatMenuExtended - Debug` sibling sub-mod** for hard-to-reach test paths. Ships one event at v0.4.1: `SCMED.1` (assigns target to dynasty 777777, used to verify the Outcast rename displays correctly). Mirror of ER's `EliteRecruitmentDebug` pattern. Enable alongside SCME in the launcher to use.

## [0.4.0] - 2026-06-12

### Fixed
- **`convert_*` orphan-building freeze** — `convert_to` leaves the old holding's buildings on the slot, which froze the game. Each `convert_*` decision now strips the source type's buildings before `convert_to` via new helpers in `common/scripted_effects/scme_holding_effects.txt` (one `strip_<type>_buildings_effect` per source type: castle, temple, city, tribal, nomad). The Aztec rename (`ca_culture_nahuatl_*` / `ca_culture_nahua_*`) is flag-gated.
...
### Added
- **CleanSlate trait-ID compat hooks** for the 22 vanilla trait IDs CleanSlate renames that SCME touches, routed via `has_global_flag = cleanslate_active`. Two new directories: `common/scripted_effects/` (`scme_trait_compat.txt`, `scme_spawn_effects.txt`, `scme_health_effects.txt`) and `common/scripted_triggers/` (`scme_trait_compat_triggers.txt`, `scme_health_triggers.txt`).
...
- **Mod files** — launcher `SketchyCheatMenuExtended.mod` + descriptor `SketchyCheatMenuExtended/descriptor.mod`. Renamed to "Sketchy Cheat Menu Extended", replaced the upstream tag list with `{ "Decisions" "Cheats" "QoL" "Compatibility" }`, added `dependencies = { "CleanSlate" }` so CleanSlate's `cleanslate_active` flag is set before this mod loads.
